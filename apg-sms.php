...
if ( is_plugin_active( 'woocommerce/woocommerce.php' ) || is_network_only_plugin( 'woocommerce/woocommerce.php' ) ) {
   ...
   * @return bool
   */
  function jckwds_has_date_or_time( $order ) {
    $meta = array(
    	'date' => false,
    	'time' => false,
    	'slot' => false
    ...
    $date = get_post_meta( $order->id, 'jckwds_date', true);
    $time = get_post_meta( $order->id, 'jckwds_timestamp', true);
    $slot = get_post_meta( $order->id, 'jckwds_timeslot', true);
	...
	function apg_sms_procesa_estados( $pedido, $notificacion = false ) {
        ...
        //
			...
			case 'processing': //Pedido procesando
				if ( !!array_intersect( array( "todos", "mensaje_pedido" ), $apg_sms_settings['mensajes'] ) &&
            ((isset( $apg_sms_settings['notificacion'] ) && $apg_sms_settings['notificacion'] == 1) ||
             (isset( $apg_sms_settings['notificacion_today_only'] ) && $apg_sms_settings['notificacion_today_only'] == 1))
             && $notificacion ) {
          if ( !is_array( $telefono_propietario ) ) {
              ...
              if($jckwds_meta !== false && $jckwds_meta['time'] < $tomorrow) {
                  ...
                  apg_sms_envia_sms( $apg_sms_settings, $telefono_propietario, $mensaje_pedido ); //Mensaje para el propietario
              }
          } else {
              foreach( $telefono_propietario as $administrador ) {
                  apg_sms_envia_sms( $apg_sms_settings, $administrador, apg_sms_procesa_variables( $mensaje_pedido, $pedido, $apg_sms_settings['variables'] ) ); //Mensaje para los propietarios
