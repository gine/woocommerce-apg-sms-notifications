    ...
    <table class="form-table apg-table">
      ...
      <tr valign="top">
        <th scope="row" class="titledesc"> <label for="apg_sms_settings[notificacion_today_only]">
            <?php _e( 'New order notification today only:', 'woocommerce-apg-sms-notifications' ); ?>
          </label>
          <span class="woocommerce-help-tip" data-tip="<?php _e( "Check if you want to receive a SMS message when there's a new order only wen the order must be delivered today", 'woocommerce-apg-sms-notifications' ); ?>"></span> </th>
        <td class="forminp forminp-number"><input id="apg_sms_settings[notificacion_today_only]" name="apg_sms_settings[notificacion_today_only]" type="checkbox" value="1" <?php echo ( isset( $apg_sms_settings['notificacion_today_only'] ) && $apg_sms_settings['notificacion_today_only'] == "1" ? 'checked="checked"' : '' ); ?> tabindex="<?php echo $tab++; ?>" /></td>
      </tr>
      <tr valign="top">
